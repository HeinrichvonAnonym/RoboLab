...
from gello.zenoh_core.robot_node import DEFAULT_ROBOT_KEY, ZenohClientRobot
...
class Args:
    ...
    zenoh_connect: Optional[Tuple[str, ...]] = None
    zenoh_timeout_s: float = 5.0
...
def main(args):
    if args.mock:
        ...
        robot_client = ZenohClientRobot(
            key=args.robot_key,
            connect=args.zenoh_connect,
            timeout_s=args.zenoh_timeout_s,
        )
    ...
    arm_dofs = min(len(start_pos), len(joints)) - 1
    ...
    action_delta = action[:arm_dofs] - joints[:arm_dofs]
    if (action_delta > 0.5).any():
        print("Action is too big")

        # print which joints are too big
        joint_index = np.where(action_delta > 0.8)
