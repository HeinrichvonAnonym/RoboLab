...
from gello.zenoh_core.robot_node import DEFAULT_ROBOT_KEY, ZenohServerRobot
...
class RobotServerThread(threading.Thread):
...
class MujocoRobotServer:
    def __init__(
        ...
        zenoh_connect: Optional[Sequence[str]] = None,
        ...
        self._gripper_ctrlrange = None
        if self._num_joints > 0:
            ctrlrange = self._model.actuator_ctrlrange[self._num_joints - 1]
            if not np.allclose(ctrlrange, [0.0, 1.0]):
                self._gripper_ctrlrange = ctrlrange

        self._joint_state = np.zeros(self._num_joints)
        self._joint_cmd = self._joint_state

        del host, port
        self._robot_server = ZenohServerRobot(
            robot=self, key=robot_key, connect=zenoh_connect
        )
        self._robot_server_thread = RobotServerThread(self._robot_server)
    ...
    def command_joint_state(self, joint_state: np.ndarray) -> None:
        ...
        if self._gripper_ctrlrange is not None:
            _joint_state = joint_state.copy()
            gripper_min, gripper_max = self._gripper_ctrlrange
            _joint_state[-1] = gripper_min + _joint_state[-1] * (
    ...
    def stop(self) -> None:
        if hasattr(self, "_robot_server"):
            self._robot_server.stop()
