#include<speed_adaptive_control/rl_executer.h>
                ...
                JacobianCalculator& jac)
{
    ...
    rl_raw_action_sub = nh.subscribe("rl_raw_action", 10, &MPC_Governor::rl_raw_action_callback, this);
}
...
void MPC_Governor::rl_raw_action_callback(const std_msgs::Float32MultiArray::ConstPtr& msg)
{
    for(int i=0;i<7;i++){
        rl_raw_action[i] = msg->data[i];
    }
    got_rl_raw_action = true;
}
...
{   
    ...
    ac->osc_control(rl_raw_action, query_q);
